...
def calculator_UI(frame,button_list):
    ...
    cv2.line(mask , (700, 0), (700, 700), (255, 255, 255), 2)
    ...
    try:
        ...
        if value != '=' and value != 'C' and value != 'P' and value != 'E':
            ...
            return "".join(str(i) for i  in exp_list)
                ...
                if value == 'E':
                    threading.Thread(target=speak, args=("drawing erase",)).start()
                    flag=1
                    mask=np.zeros((720,1280,3),np.uint8)
                    return "".join(str(i) for i  in exp_list)
                elif value == 'P':
                    ...
                    input_img = cv2.resize(mask, (1280, 720))  # Optional if mask might not be this size
                    input_img = input_img / 255.0  # Normalize like during training
                    input_img = np.expand_dims(input_img, axis=0)  # Shape: (1, 720, 1280, 3)

                # Predict
                    pred = model.predict(input_img)
                    digit = np.argmax(pred)
                    threading.Thread(target=speak, args=(digit,)).start()
                #pred = model.predict(mask)
                    exp_list.append(digit)
                    return "".join(str(i) for i in exp_list)
